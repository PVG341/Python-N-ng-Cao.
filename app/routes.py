        ...
        try:
            with engine.connect() as connection:
                insert_query = users_table.insert().values(
                    ...
                    user_role=0,  # Mặc định là người dùng
                    address=None
                )
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if user:
                ...
                session['auth'] = True
                session['user_role'] = user.user_role  # Lưu user_role vào session
...
def get_cart():
    ...
    try:
        with engine.connect() as connection:
            query = select(
                carts_table.c.cart_id,
                carts_table.c.prod_id,
                carts_table.c.prod_qty,
                products_table.c.product_name,
                products_table.c.product_price,
                products_table.c.product_img
            ).where(
                (carts_table.c.user_id == user_id) &
                (carts_table.c.prod_id == products_table.c.id)
            )
            result = connection.execute(query)
            cart_items = [
                {
                    ...
                    "prod_img": url_for('static', filename=f"uploads/{row.product_img}")
                } for row in result
            ]
        ...
        with engine.connect() as connection:
            ...
            if existing_cart_item:
                ...
                update_query = carts_table.update().where(
                    carts_table.c.cart_id == existing_cart_item.cart_id
                ).values(prod_qty=carts_table.c.prod_qty + prod_qty) 
    ...
    try:
        with engine.connect() as connection:
            ...
            if existing_cart_item:
                ...
                update_query = carts_table.update().where(
                    carts_table.c.cart_id == existing_cart_item.cart_id
                ).values(prod_qty=prod_qty)  # Cập nhật số lượng mới
                connection.execute(update_query)
...
def search():
    ...
    query = products_table.select().where(products_table.c.product_name.ilike(f'%{search_query}%'))
...
def show_category(id, category_name):
    ...
    with engine.connect() as connection:
        ...
        query = select(products_table).where(
            products_table.c.cat_id == id,
        ).limit(per_page).offset(offset)
        result = connection.execute(query).mappings()
        products = [dict(row) for row in result]

        # Tính tổng số trang
        total_products_query = select(func.count(products_table.c.id)).where(products_table.c.cat_id == id)
        total_products_result = connection.execute(total_products_query).scalar()
        total_pages = ceil(total_products_result / per_page)
...
def admin_board():
    ...
    engine = create_engine(f'postgresql+psycopg2://postgres:12345@localhost:5432')
    try:
        with engine.connect() as connection:
            databases = ["KTCK"]
    except SQLAlchemyError as e:
    ...
    try:
        ...
        with engine.connect() as connection:
            update_query = products_table.update().where(products_table.c.id == product_id).values(
                ...
                status=status,
                cat_id=category_id,
                product_img=image_filename if image_filename else products_table.c.product_img
            )
            connection.execute(update_query)
            connection.commit()
...
def delete_product():
    try:
        ...
        with engine.connect() as connection:
            ...
            query = select(products_table.c.product_img).where(products_table.c.id == product_id)
            result = connection.execute(query).fetchone()
...
def get_category():
    try:
        ...
        with engine.connect() as connection:
            query = categories_table.select().where(categories_table.c.cat_id == category_id)
            result = connection.execute(query).fetchone()
...
def update_category():
    try:
        ...
        with engine.connect() as connection:
            update_query = categories_table.update().where(categories_table.c.cat_id == category_id).values(
                name=name,
                status=status,
            )
            connection.execute(update_query)
...
def delete_category():
    try:
        ...
        with engine.connect() as connection:
            # Xóa sản phẩm trong cơ sở dữ liệu
            delete_query = categories_table.delete().where(categories_table.c.cat_id == category_id)
            connection.execute(delete_query)
            connection.commit()
